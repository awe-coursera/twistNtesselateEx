	...
	console.log('initTexture called to read raw file');
	var files = document.getElementById('files').files;
    	...
    	var file = files[0];
	console.log('we have a raw file');
	...
	reader.onloadend = function(e) {
	    ...
	    var resultDataView = new DataView(reader.result);
	    console.log("resultDataView length ", resultDataView.byteLength);
	    checkerBoardTexture = new Uint8Array(resultDataView.byteLength);
	    for (var i=0 ; i<resultDataView.byteLength; i++) {
  		checkerBoardTexture[i] = resultDataView.getUint8(i);
	    }

	    console.log("checkerBoardTexture length", checkerBoardTexture.length);
        };
	...
	sphereTexture = gl.createTexture();
    	...
    	//gl.texParameteri( gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
	gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, 1);
    	gl.texImage2D( gl.TEXTURE_2D, 0, gl.RGBA, 128, 128, 0, gl.RGBA, gl.UNSIGNED_BYTE, checkerBoardTexture);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR_MIPMAP_NEAREST);
	...
	// Trying to render computed checkerboard pattern 
